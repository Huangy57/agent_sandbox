...
- **Stub srun:** Proxied through the chaperon like sbatch. On the login node (no `SLURM_JOB_ID`), the chaperon handler rejects the request. Inside a Slurm allocation (compute node), the handler validates flags against a whitelist and execs the real srun for job-step launching (MPI, multi-process). The chaperon runs outside the sandbox and has munge access — munge is never exposed inside the sandbox. Dangerous flags (`--jobid`, `--uid`, `--export`, `--container`, allocation flags) are always blocked.
