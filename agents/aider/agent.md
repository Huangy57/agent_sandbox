...
- **Stateful experimentation:** For iterative work with expensive state (large dataframes, trained models), `lab` starts a persistent JupyterLab in the project directory; attaching to its kernel from each turn lets state survive between turns. Run `lab help` for details.
